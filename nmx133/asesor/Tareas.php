...
*/
	...
	$sq3.= "FROM TS_Tareas ";
	$sq3.= "WHERE Tipo=1 AND AsesorId=$EMP  ";
...
<?		if($Nuevo==0){ ?>
		<form method="post" action="Tareas_Guardar.php" name="FrmTareas">
		<input type="hidden" name="idt" value="<?=$TID?>">
			...
			<tr><td valign="top" align="left">
				...
				<tr><td bgcolor="<?=CLR_CAB_DET?>" align="right"><font class="textcampo">Respuesta :</td>
					<td align="left">
					<? if($TaEst==0){ ?><textarea name="resp" class="txt00"></textarea><? }else{ ?><?=$Respu?><? } ?></td>
				</tr>
			...
			<tr><td align="center">
<?	if($TaEst==0){	?>
					<input type="submit" class="btn00" value="Responder Tarea">
<?	}?>
